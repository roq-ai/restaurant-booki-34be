...
function DashboardPage() {
  ...
  const menuItemStat = useSWR<PaginatedInterface<MenuItemInterface>>(
    () => `/menu-items`,
    () => getMenuItems({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Menu Items',
      result: menuItemStat?.data?.totalCount,
      isLoading: menuItemStat.isLoading,
